...
const STEPS: { step: number; label: string; screen: Screen }[] = [
  ...
  { step: 4, label: '4 · Compartilhar', screen: 'save-teaser' },
];
  ...
  return (
    <nav className="flex flex-wrap gap-3">
      {STEPS.map(({ step, label, screen: target }) => {
        ...
        // Navegável só para trás ou até onde o usuário já chegou (nunca além da etapa 3)
        const clickable = step !== current && step <= Math.min(maxStep, 3);
        const palette =
          state === 'current'
            ? 'bg-ink text-paper'
            : state === 'done'
